...
@mcom =
  ...
  onMenuHandler:
    ontouchend: (e)-> mcom.onMenuOpen(e)
  Main:
    view: ->
      m '#viewport',
        m '#page-main.page.current-page',
          style:
            backgroundColor: theme.default.bg
          m '.menu.txt', mcom.onMenuHandler,
            m '.wrapper'
              m 'i.ion-ios-gear-outline'
          m '.quote-list',
            m '.quote-wrapper.active',
              m '.quote.txt', '"서두르지 말라. 그러나 쉬지도 말라."'
              m '.author.txt', '- 괴테'
            m '.quote-wrapper',
              m '.quote.txt', ' "가난한 사람이란 적게 가진 자가 아니라, 너무 많이 가지려는 자이다."'
              m '.author.txt', '- 세네카'
            m '.quote-wrapper',
              m '.quote.txt', '"시작이 반이다."'
              m '.author.txt', '- 아리스토텔레스'
            m '.quote-wrapper',
              m '.quote.txt', '"슴가 하앍.. 낼름.."'
              m '.author.txt', '- 말리부 스튜디오'
        m '#page-settings.page',
          m '.close',
            m 'i.ion-ios-close-empty'
          m '.settings-container',
            m '.settings-title', '테마선택'
